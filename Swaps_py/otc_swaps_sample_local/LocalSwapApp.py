...
class LocalSwaps():
        ...
        try:
                    ...
                    for i in on_time_tenors:
                        duplicated_tenor[i] = [k.replace('X','') for k in list(
                            chain.from_iterable([list(filter(lambda x: 'X' in x,re.split(i,j))) \
                                                 for j in spreads_on_time.tenor.unique().tolist() if i in j]))]
                    tenors_to_search = list(set(list(chain.from_iterable(list(duplicated_tenor.values())))))
            ...
            if not best_quotes.empty:
                ...
                if best_ask_and_bid==[]:
                    ...
                    #best_active_quote = (best_active_quote.reset_index('tipo-orden')[check_ba_filter.ba_spread<=(
                    #    check_ba_filter.loc[:,0]+0.00000001)]).set_index('tipo-orden', append = True)
            ...
            if brokers_matchs_spreads_off_time.empty or all(x.empty for x in one_to_third_tenors_list):
                ...
                if spreads_off_time.empty:
                    ...
                    for i in one_to_third_tenors:
                        duplicated_tenor[i] = [k.replace('X','') for k in list(
                            chain.from_iterable([list(filter(lambda x: 'X' in x,re.split(i,j))) \
                                                 for j in spreads_off_time.tenor.unique().tolist() if i in j]))]
                    tenors_to_search = list(set(list(chain.from_iterable(list(duplicated_tenor.values())))))
    ...
    def bid_ask_spread_builder(self, last_bid_ask_spread_hist):
        ...
        try:
            ...
            prev_info = last_bid_ask_spread_hist.loc[last_bid_ask_spread_hist.tenor.isin(no_ba_spread_today), ['bid-ask-spread']]
