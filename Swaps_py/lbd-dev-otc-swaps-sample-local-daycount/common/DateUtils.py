...
class DateUtils():
    ...
    def previous_business_day_convention(self, date, calendar):
        ...
        while flag:
            ...
            if (offset_date in calendar):
                flag = False
            offset = offset - 1
            previous_business_day_convention = offset_date
    ...
    def following_business_day_convention(self, date, calendar):
        ...
        while flag:
            ...
            if (offset_date in calendar):
                flag = False
            offset = offset + 1
            following_business_day_convention = offset_date
    ...
    def modified_previous_business_day_convention(self, date, calendar):
        ...
        :return: La fecha de pago en formato datetime.date
        """
        previous_business_day = self.previous_business_day_convention(date, calendar)
        following_business_day = self.following_business_day_convention(date, calendar)
...
class CalendarHandler():
    ...
    Args:
        self.val_date (dt.datetime): Fecha de valoracion
        self.logger (logging.RootLogger): Logger con nivel predeterminado para la escritura de errores e informacion
        self.prev_month (int): Numero de meses previos a la fecha de valoracion, indicando el inicio del calendario
        self.max_years (int): Maximo numero de años a futuro a consultar del calendario, indicando el fin del mismo
        self.chunk_size (int): Tamaño de la busqueda de las fechas de la BD de calendarios (por paginacion usualmente es 1000)
        self.has_more (bool): Indicador que evidencia si la busqueda del calendario en el rango seleccionado tiene un numero mas grande que el chunk_size inicial
    """
    def __init__(self, val_date, logger, prev_month = 1,max_years = 120, chunk_size = 1000, has_more = True):
    ...
    def get_last_months_calendar(self, calendar_dt):
        """Obtiene el calendario de ultimos dias habiles por mes, teniendo inicalmente un calendario enlistado.
        Este se convierte a calendario enlistado datetime.datetime, se convierte en DatFrame, 
        se agrupa por año y por mes, se agrega por la fecha maxima y se devuelve el calendario resultante
        
        Args:
            calendar (list): Calendario especifico enlistado
        Returns:
            max_montly_bussiness_cal (list): Listado de fechas en formato datetime.datetime con el calendario de ultimo dia habil mensual
        """
        try:
            ...
            self.logger.error('Se genero un error creando el calendario de ultimos dias habiles por mes. Fallo linea: '+ error_line)
            self.logger.error(e)
