...
class LocalOptions():
    ...
    def USDCOP_strategy_surface_sample(self):
        ...
        try:
            ...
            brokers_matchs_on_time = broker_info.loc[((broker_info['hora-inicio-operacion'] >= start_hour )\
            ...
            brokers_match_out_time = (broker_info.drop(brokers_matchs_on_time.index.to_list(), 
                                                               axis=0)).loc[((broker_info['hora-inicio-operacion']<start_hour) \
                                                                            | (broker_info['hora-fin-operacion']>closing_hour)) \
                                                                            & (broker_info['orden'] == 'TRADE')]
            ...
            if best_data.empty:
                ...
                non_best_quotes = list(set(best_data.reset_index('orden').index.to_list()) & \
                                        set(best_data_on_time.reset_index('orden').index.to_list() + \
                                        best_data_out_time.reset_index('orden').index.to_list()))
                best_data.drop(index=[(i[0], i[1]) for i in non_best_quotes], inplace = True)
                self.logger.info('Informacion best data clean: \n ' +
                             best_data.to_string())
                best_ask_and_bid = []
            ...
            self.logger.info('Informacion best trade on time: \n ' +
                             best_data_ba.to_string())
            ...
            if not best_data_on_time.empty:
                ...
                self.logger.info('Informacion opc trade on time: \n ' +
                             opc_trades_on_time.to_string())
            self.logger.info('Informacion muestra: \n ' +
                             opc_today.to_string())           
