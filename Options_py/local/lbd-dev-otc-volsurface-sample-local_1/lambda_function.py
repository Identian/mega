...
def lambda_handler(event, context):
    ...
    try:
        ...
        s_val_date = event['valuation_date']
        val_date = dt.datetime.strptime(s_val_date, '%Y-%m-%d').date()
    ...
    return {"statusCode": 200,
                "body":{
                    'opt_strategies': opc_strategies,
                    'opt_deltas': opc_deltas,
                    'spr_USDCOP': spread_jumps_dict,
                    'd_opciones_citi': days_citi_dict
        }
    }