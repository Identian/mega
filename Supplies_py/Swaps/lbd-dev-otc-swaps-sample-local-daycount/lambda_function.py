...
try:
    ...
    logger.error(
        "Se genero un error cargando las variables de entorno. Fallo linea : "
        + error_line
    )
    logger.error(e)
...
def lambda_handler(event, context):
    ...
    """
    err_mss = (
        "Se genero un error en el calculo de dias de la muestra swap. Fallo linea : "
    )
    try:
        try:
        ...
        logger.info(
            "Inicio calculo de dias bursatiles hasta el pago del tenor para el swap: "
            + swap
            + ". Para el: "
            + dt.datetime.strftime(val_date, "%Y-%m-%d")
        )
        swap_cal = list(event["calendars"].keys())[0]
        swap_starting_convention = json.loads(
            os.environ["SWAP_STARTING_DAY_CONVENTION"]
        )[swap]
        swap_business_convention = json.loads(
            os.environ["SWAP_BUSINESS_DAY_CONVENTION"]
        )[swap]
        ...
        on_day, tenor_days = DayCount().local_swaps_daycount(
            val_date,
            calendar_dt,
            swap_nodes_copy,
            swap_starting_convention,
            swap_business_convention,
        )
    ...
    return {"body": {"on_day": on_day, "swap_days": tenor_days}}
