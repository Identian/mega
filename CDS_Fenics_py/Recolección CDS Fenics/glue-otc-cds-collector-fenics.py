"""
Módulo para la recolección del archivo CDS del vendor Fenics
...
def update_report_process(status, description, technical_description, filename):
    ...
    report_process = {
          ...
          "stage": "Recolección",
          "status": "",
          "aws_resource": "glue-p-otc-cds-collector-fenics",
          "type": "insumo",
          "description": "",
...
Puede encontrar el log para mas detalles de las siguientes maneras:
    ...
    - Ir al servicio AWS Glue desde la consola AWS, dar clic en 'Jobs'(ubicado en el menu izquierdo), dar clic en el job 'glue-p-otc-cds-collector-fenics', dar clic en la pestaña 'Runs' (ubicado en la parte superior), y buscar la ejecucion asociada a este mensaje por instrumento, fecha de valoracion y error comunicado, para ver el log busque el apartado 'Cloudwatch logs' y de clic en el link 'Output logs'.
...
def main():
    ...
    name_file_cds = params_glue["FILE_NAME_CDS"]
    ...
    subject_error = "Error en la colección del archivo CDS de Fenics"
    body_error = "La recolección del archivo de CDS del SFTP de Fenics ha fallado: "
    subject_succesful = "Recolección exitosa de archivo CDS SFTP Fenics"
    body_succesful = "La recolección del archivo de CDS del SFTP de Fenics ha sido exitosa: "
    email = ReportEmail()
    ...
    file_name = name_file_cds.replace('ddmmyy', valuation_date)

    route_cds = route_cds+file_name
    email = ReportEmail()
    try:
        sftp_reader = SFTPFileReader(data_connection_sftp)
        sftp_reader.connect_to_sftp()
        file_data = sftp_reader.download_file_sftp(route_cds)
