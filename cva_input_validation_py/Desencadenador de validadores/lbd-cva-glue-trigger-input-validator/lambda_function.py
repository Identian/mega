...
def identify_job_name(glues_dict: dict, path: str):
    try:
        PRODUCTS_DICT = {
            "forwards": "GLUE_FWD",
            "opciones": "GLUE_OPT",
        ...
        job_name = "No identificado"
        if "forwards" in path:
            job_name = glues_dict[PRODUCTS_DICT["forwards"]]
        elif "opciones" in path:
            job_name = glues_dict[PRODUCTS_DICT["opciones"]]
        elif "swaps_cf" in path:
            job_name = glues_dict[PRODUCTS_DICT["swaps"]]
        else:
            msg = "El producto no esta en la lista de productos que se validan "
            msg+= "actualmente, o la ruta o nombre del archivo son incorrectos"
            msg += "(los archivos swap no desencadenan la validacion, para "
            msg += "validarlos hay que cargar ambos archivos: swaps y swaps_cf)"
            msg += ". Valide el archivo que esta cargando en el bucket"
            raise PlataformError(msg)
        return job_name
    except (Exception,) as gen_exc:
        raise_msg = "Fallo La identificacion del producto"
        error_msg = create_log_msg(raise_msg)
        logger.error(error_msg)
        raise PlataformError(raise_msg) from gen_exc
...
def lambda_handler(event, context):
    ...
    try:
        ...
        glues_dict = {
            "GLUE_FWD": os.environ["GLUE_FWD"],
            "GLUE_OPT": os.environ["GLUE_OPT"],
            "GLUE_SWP": os.environ["GLUE_SWP"],
        }
        name_glue_job = identify_job_name(glues_dict=glues_dict, path=s3_full_path)
