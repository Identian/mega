# -*- coding: utf-8 -*-
"""
Created on Fri Nov  4 10:11:06 2022
...
class DateUtils:
    ...
    def end_of_month_business_day_convention(self, date, calendar):
        ...
        """
        first_date_next_month = date.replace(day=28) + dt.timedelta(
            days=4
        )  # this will never fail
        end_of_month_business_day_convention = first_date_next_month - dt.timedelta(
            days=first_date_next_month.day
        )
    ...
    def end_of_month_no_adjustment_business_day_convention(self, date, calendar):
        ...
        """
        end_of_month_no_adjustment_business_day_convention = (
            self.end_of_month_business_day_convention(date, calendar)
        )
    ...
    def end_of_month_previous_business_day_convention(self, date, calendar):
        ...
        :return: Fecha de pago en formato datetime.date
        """
        end_of_month_day_convention = self.end_of_month_business_day_convention(
            date, calendar
        )
        previous_business_day = self.previous_business_day_convention(
            end_of_month_day_convention, calendar
        )
        following_business_day = self.following_business_day_convention(
            end_of_month_day_convention, calendar
        )
    ...
    def days_non_leap(self, initial_date, final_date):
        ...
        rs.rrule(
            rrule.rrule(
                rrule.YEARLY,
                dtstart=initial_date + dt.timedelta(days=1),
                until=final_date,
                bymonth=2,
                bymonthday=29,
            )
    ...
    def day_count_act_365(self, initial_date, final_date):
        ...
        day_count_act_365 = (
            sum(leap_years) / 366 + (len(leap_years) - sum(leap_years)) / 365
        )
    ...
    def tenor_specs(self, tenor):
        ...
        tenor_raw_length = (
            int(re.sub(tenor_raw_period, "", tenor))
            if re.sub(tenor_raw_period, "", tenor).isdigit()
            else 1
        )
    ...
    def tenor_date(
        ...
        tenor_date = self.business_date(
            tenor_raw_date, calendar, business_day_convention
        )
        return tenor_date

    def tenor_sequence_dates(
        self,
        trade_date,
        tenor,
        frequency,
        ...
        :return: Fecha correspondiente de un plazo de mercado segun convencion con formato datetime.date
        """
        effective_date = self.starting_date(
            trade_date, business_day_calendar, starting_day_convention
        )
        tenor_specs = self.tenor_specs(tenor)
        frequency_specs = self.tenor_specs(frequency)
        ...
        tenors = [
            str(x) + frequency_specs["period"]
            for x in range(
                frequency_specs["length"],
                frequency_specs["length"] + (tenor_count * frequency_specs["length"]),
                frequency_specs["length"],
            )
        ]
        tenors_dates = [effective_date] + [
            self.tenor_date(
                trade_date,
                x,
                business_day_calendar,
    ...
    def month_third_wednesday(
        self, date_to_adjust, calendar_business_days=None, prior_day_adjust=None
    ):
        """
        ...
        if prior_day_adjust is not None:
            third_wednesday = self.add_business_days(
                third_wednesday, prior_day_adjust, calendar_business_days
            )
        return third_wednesday

    def uf_forward_date(self, trade_date, tenor, business_day_calendar):
        next_date = self.following_business_day_convention(
            trade_date, business_day_calendar
        )
        uf_end_date = dt.date(next_date.year, next_date.month, 9)
        uf_publish_date = self.previous_business_day_convention(
            uf_end_date, business_day_calendar
        )
        if (uf_publish_date - trade_date).days <= 0:
            uf_next_date = uf_end_date + rd.relativedelta(months=1)
        else:
            uf_next_date = uf_end_date
        tenor_raw_date = self.add_tenor(uf_next_date, tenor)
        tenor_date = self.previous_business_day_convention(
            tenor_raw_date, business_day_calendar
        )
        return tenor_date
...
class CalendarHandler:
    ...
    def get_last_day_month(self, trade_date):
        ...
        try:
            ...
            self.logger.error(
                "Se genero un error en la extracción del ultimo dia del mes"
            )
            raise Exception(
                "Hubo un error en la extracción del ultimo dia del mes"
            ) from exc
    ...
    def get_last_buss_day_months(self, calendar_dt):
        ...
        try:
            ...
            max_montly_bussiness_days = (
                df_cal.groupby(
                    [pd.DatetimeIndex(df_cal).year, pd.DatetimeIndex(df_cal).month]
                )
                .max()
                .reset_index(drop=True)
            )
            max_montly_bussiness_cal = [
                x.to_pydatetime().date() for x in max_montly_bussiness_days.tolist()
            ]

            return max_montly_bussiness_cal
        except (Exception,) as exc:
            self.logger.error(
                "Se genero un error en la extracción del ultimo dia habil para los meses del calendario"
            )
            raise Exception(
                "Hubo un error en la extracción del ultimo dia habil para los meses del calendario"
            ) from exc
    ...
    def get_first_buss_day_months(self, calendar_dt):
        ...
        try:
            ...
            min_montly_bussiness_cal = [
                x.to_pydatetime().date() for x in min_montly_bussiness_days.tolist()
            ]

            return min_montly_bussiness_cal
        except (Exception,) as exc:
            self.logger.error(
                "Se genero un error en la extracción del primer dia habil para los meses del calendario"
            )
            raise Exception(
                "Hubo un error en la extracción del primer dia habil para los meses del calendario"
            ) from exc
